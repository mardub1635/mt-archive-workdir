...
with open(FILE_NAME+".tsv", 'w') as f:
    ...
    for p in page_lines:
        ...
        try:
            title_presentation = ":".join(p.text.split(":")[1:])
            try:
                title = title_presentation.split(";")[0]
                presentation= has_pdf(p,1)
                presentation=urljoin(URL,presentation)
            except:

                title=title_presentation
                print(title_presentation)
        except:
            continue
        # try:
        #     print(title)
